...
export default function Squares() {
  ...
  const getClassName = id => {
    ...
    return id === activeSquare ? 'active' : ''; // shorthand for full if else statement
  };
      ...
      <div className='squares'>
        {
          ...
          squares.map(id => // changed listOfSquaresIds to squares
            <div
              id={id}
              key={id}
              className={`square ${getClassName(id)}`}
              onClick={() => markActive(id)} // adding () => to trick JS to run it when called
            >
            </div>
          )
        }
