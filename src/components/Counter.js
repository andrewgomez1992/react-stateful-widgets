...
import React, { useState } from 'react'; /* STEP 0 */ // importing the state hook, useState

export default function Counter() {
  /* STEP 1 */
  const [count, setCount] = useState(0); // creating count and setCount pair, the count state is initialized to 0

  const increment = () => {
    /* STEP 4 */
    setCount(count + 1) // NEVER SET STATE DIRECTLY I.E count++
  ...
  const reset = () => {
    /* STEP 6 */
    setCount(0); // setting the reset button to 0
  };

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: count % 2 === 0 ? 'royalblue' : "crimson" /* STEP 2 */ // equal to if statement, CLEANEST CODE
  };
